...
async function getBaseMarketIntelligence() {
    ...
    try {
        ...
        if (llamaRes.ok) {
            ...
            const baseChain = chains.find((c) => c.name?.toLowerCase() === 'base');
        ...
        if (dexRes.ok) {
            ...
            for (const core of CORE_BASE_TOKENS) {
                const matching = pairs
                    .filter((p) => p.baseToken?.address?.toLowerCase() === core.address.toLowerCase() && p.chainId === 'base')
...
async function getRecentBaseWhaleTransactions(limit = 5) {
    const ETH_PRICE_ESTIMATE = 2524.00;
    const whaleTxs = [];
    try {
        const latestBlockNumber = await blockchain_1.publicClient.getBlockNumber();
        // Scan last 15 blocks
        const blockPromises = [];
        for (let i = 0; i < 12; i++) {
            blockPromises.push(blockchain_1.publicClient.getBlock({
            ...
            for (const tx of b.transactions) {
                ...
                if (valETH >= 1.0) {
                    whaleTxs.push({
                        ...
                        explorerUrl: `${shared_1.BASE_EXPLORER_URL}/tx/${tx.hash}`
                    });
                }
            }
        }
